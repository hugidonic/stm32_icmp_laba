...
void dhcp_filter(eth_frame_t *frame, uint16_t len) {
  ...
  if ((len >= sizeof(dhcp_message_t)) && (dhcp->operation == DHCP_OP_REPLY) &&
    ...
    if (!renew_server) renew_server = ip->from_addr;
    ...
    switch (type) {
      ...
      case DHCP_MESSAGE_ACK:
        if ((dhcp_status == DHCP_WAITING_ACK) && (lease_time)) {
          if (!renew_time) renew_time = lease_time / 2;
...
uint8_t tcp_open(uint32_t addr, uint16_t port, uint16_t local_port) {
  ...
  if (st) {
    ...
    tcp->cksum = ip_cksum(len + IP_PROTOCOL_TCP, (uint8_t *)tcp - 8, len + 8);
...
void tcp_filter(eth_frame_t *frame, uint16_t len) {
  ...
  tcpflags =
      tcp->flags & (TCP_FLAG_SYN | TCP_FLAG_ACK | TCP_FLAG_RST | TCP_FLAG_FIN);
...
void tcp_poll() {
  ...
  for (id = 0; id < TCP_MAX_CONNECTIONS; ++id) {
    ...
    if ((st->status != TCP_CLOSED) && (rtime() - st->rx_time > TCP_TIMEOUT)) {
      st->status = TCP_CLOSED;
      tcp_closed(id, 1);
    }
  }
...
void icmp_filter(eth_frame_t *frame, uint16_t len) {
  ...
  LcdGotoXY(0, 0);
  LcdSendString("  ICMP packet!  ", 16);
  LcdGotoXY(0, 1);
  LcdSendString("                ", 16);
  sprintf(Buffer, "Len:%03d,Seq:%04d", len, (icmp->seq));
  LcdGotoXY(0, 2);
...
uint8_t ip_send(eth_frame_t *frame, uint16_t len) {
  ...
  if (ip->to_addr == ip_broadcast) {
    ...
    // resolve mac address
    if (!(mac_addr_to = arp_resolve(route_ip))) return 0;
...
uint8_t *arp_resolve(uint32_t node_ip_addr) {
  ...
  // search arp cache
  if ((mac = arp_search_cache(node_ip_addr))) return mac;
      ...
      switch (msg->type) {
        ...
        case ARP_TYPE_RESPONSE:
          if (!arp_search_cache(msg->ip_addr_from)) {
            ...
            arp_cache_wr++;
            if (arp_cache_wr == ARP_CACHE_SIZE) arp_cache_wr = 0;
          }
          break;
      }
    }
...
uint8_t lan_up() { return ip_addr != 0; }
